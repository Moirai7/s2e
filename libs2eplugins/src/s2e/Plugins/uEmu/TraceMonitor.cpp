...
void TraceMonitor::onInvalidLoopDetection(S2EExecutionState *state, uint64_t pc, unsigned source_type) {
    DECLARE_PLUGINSTATE(TraceMonitorState, state);
    // cache mode
    getWarningsStream() << " current state size = " << state->stack.size()
                                     << "\n";
    if (pc == 0x08001e7e) { //trace里面invalid里面加
    ...
    std::vector<uint32_t> conregs = getRegs(state, pc);//当前state。PC下的寄存器
    //日志打印位置
    ...
    // if already at least one tb is same, compare other tbs in loop
        ...
        if (k == conregs.size()) {
            ...
            if (plgState->getloopsize() == 0) {
                // if (plgState->getmodeflag()) {
                    ...
                    getInfoStream() << "Kill State due to dead loop (multi-tbs) " << "\n";
    ...
    for (i = 0; i < plgState->getcachesize(); ++i) {
        ...
        if (j == conregs.size()) { // TODO: if only one reg is different, we should also go to long loop check.
            if (i == plgState->getcachesize() - 1) {
                ...
                //     single_dead_loop[pc]++;
                getInfoStream() << "Kill State due to dead loop (single tb) " << "\n";
